...
void InsertionSort(int* numbers, int numbersSize) {
   for (int i = 1; i < numbersSize; i++) {
      int j = i;
      COMPARES++; // nest line is a compare whether true or false
      while (j > 0 && numbers[j] < numbers[j - 1]) {
         ...
         j--;
         if (DEBUG) {
        cout << "\t" << "swapped elements " << numbers[j] << " and " << numbers[j-1] << endl;
...
int main() {
   ...
   cout << "Total swaps: SWAPS: " << SWAPS << endl;
   cout << "Total compares: " << "COMPARES: " << COMPARES << endl;
}
